...
contract FeatureClaimRandom {
    ...
    //feature id - 4 = dungeon
    uint256 internal constant FID = 4;
    ...
    string[2] internal CLAIMID = ["SRD.DMD","SRD.RLC0"];
    //between 93 - 108, avg 100 , then multiply by 10 
    uint256[2] internal MIN = [90,90];
    uint256[3][2] internal DICE = [[1,3,6],[1,3,6]];
    uint256[2] internal MULTI = [10,10];
        ...
        internal 
    {
        ...
        //get feature 
        (uint256 _fid, bytes32 hash) = FEATURES.featureByIndex(nft, id, fi);
        require(hash != bytes32(0), "FeatureClaim: beyond available features");
        require(_fid == FID, "FeatureClaim: incorrect feature type for cliam");
        ...
        internal view returns (uint256 d)
    {
        ...
        //use dice 
        (,d) = RND.dice(rand, DICE[ci][1], DICE[ci][2]);
    } 
        ...
        public 
    {
        ...
        uint256 _ci = RND.integer(keccak256(abi.encode(blockhash(block.number-1), block.timestamp, msg.sender, "claim")), CLAIMID.length); 
        
        //get amt to reward 
        uint256 amt = (MIN[_ci] + _delta(_ci)) * MULTI[_ci];
